...
if ( ! class_exists( 'WP_Async_Task' ) ) {
	abstract class WP_Async_Task {
        ...
        public function dispatch() {
            if ( empty( $this->data ) ) {
                throw new Exception( 'No data to pass to async task.' );
            }

            return as_enqueue_async_action( $this->action, $this->data );
        }
		...
		public function handle() {
            add_action( "wp_async_$this->action", array( $this, 'run_action' ) );
        }
	}
