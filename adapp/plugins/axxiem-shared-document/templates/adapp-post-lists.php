...
<div class="adapp-leftbar">
		<div class="blog-posts">
			...
			<div class="post-listing">
				<ul>
					 <?php if (!empty($posts_query->have_posts())) { while ( $posts_query->have_posts() ) {
        			...
        			<?php } } ?>
					 ...
					 <?php if (!empty($non_pinned_posts->have_posts())) { while ( $non_pinned_posts->have_posts() ) {
            				$non_pinned_posts->the_post();
            				$shared_post_id = get_the_ID();
            				$author_id = get_post_field('post_author', $shared_post_id);
            				// Get the author's display name
            				$author_name = get_the_author_meta('display_name', $author_id);
