...
class Axxiem_REST_API_Endpoints {
    ...
    public static function login( $request ) {
		...
		if ( is_wp_error( $user ) ) {
			...
			$simple_local_avatar = get_user_meta( $user->ID, 'simple_local_avatar', true);
			$userInfo['avatar'] = !empty($simple_local_avatar) ? $simple_local_avatar[96] : '';
	...
	public static function axxiemDashboardLists($request) {
		...
		if ($role && !in_array($role, $support_role)) {
			$dynamic_key = $role;
			...
			$meta_query = array(
				...
				array(
					'relation' => 'AND', // Combine conditions with AND (both must be true)
					// Condition 1: Check if the dynamic key exists
					array(
						'key'     => strtolower($dynamic_key),
						'compare' => 'EXISTS', // Ensure the custom field exists
					),
					// Condition 2: If it exists, apply a LIKE comparison
					array(
						'relation' => 'OR',
						array(
							'key'     => strtolower($dynamic_key),
							'value'   => $like_value,
							'compare' => 'LIKE', // Perform a LIKE comparison on the field's value
						),
						array(
							'key'     => strtolower($dynamic_key),
							'compare' => '=',
	...
	public static function axxiemPosts($request) {
		...
		$args = array(
			...
			'paged'	=> $paged,
			'orderby' => array(
				'meta_value_num' => 'DESC',   // prioritize _is_pinned = 1
				'menu_order'        => 'DESC',
				'date'              => 'DESC'
			)
		);
		...
		if (empty($directory_path)) {
			...
			if( !empty($user_type) && ($user_type == 1 || $user_type == 2)) {
				if ($role === 'supervisor') {
					...
					$restrict_dir = array('my','personnel','adapp_supervisor_information');
				}elseif ($role === 'admins') {
					$restrict_dir = array('hr');
				}else{
